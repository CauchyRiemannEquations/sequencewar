...
const BOSS_GATE_INTERVAL = 8;      // 보스전 수열 관문 주기 (초)
const BOSS_GATE_FIRST_DELAY = 4.5; // 보스전 첫 관문까지의 대기 (초)
...
let bossGateTimer = 0;  // 보스전 수열 관문 스폰 타이머 (초)
...
function startStageRuntime() {
    ...
    roadSpeed = 4.3 + currentStage * 0.12;
...
function update() {
    ...
    if (gameState === 'BOSS_BATTLE') {
        ...
        // 보스전 수열 관문: 주기적으로 문제가 내려오고,
        // 정답이면 일제사격으로 큰 피해, 오답이면 병력 손실
        if (bossCanAttack) {
            bossGateTimer += 0.016;
